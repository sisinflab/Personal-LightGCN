...
class LightGCNPModel(torch.nn.Module, ABC):
    def __init__(self,
                 num_users,
                 num_items,
                 learning_rate,
        ...
        self.Gu_p = torch.nn.Parameter(
            torch.nn.init.xavier_normal_(torch.empty((self.num_users, self.embed_k))))
        self.Gu_p.to(self.device)
        self.Gi_p = torch.nn.Parameter(
            torch.nn.init.xavier_normal_(torch.empty((self.num_items, self.embed_k))))
        self.Gi_p.to(self.device)
        self.a = a
        self.b = b
    ...
    def train_step(self, batch):
        ...
        reg_loss = self.l_w * (torch.norm(self.Gu, 2) +
                               ...
                               torch.norm(self.Gi_p, 2) +
                               torch.norm(self.Gu_p, 2))
        loss += reg_loss
